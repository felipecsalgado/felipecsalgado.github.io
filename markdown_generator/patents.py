...
for row, item in publications.iterrows():
    ...
    html_filename = str(item.year) + "-" + str(item.short_title)
    ...
    md += "\nfile_date: '" + str(item.file_date) + "'" 
    
    md += "\nissue_date: '" + str(item.issue_date) + "'"
    
    md += '\ntitle: "' + str(item.title) + '"'

    md += '\ntitle_english: "' + str(item.title_english) + '"'

    md += '\nnumber: "' + str(item.number) + '"'

    md += '\ninventors: "' + str(item.inventors) + '"'

    md += '\ncountry: "' + str(item.country) + '"'

    md += '\nlanguage: "' + str(item.language) + '"'

    if len(str(item.file)) > 5:
        md += '\nfile: "' + str(item.file) + '"'
